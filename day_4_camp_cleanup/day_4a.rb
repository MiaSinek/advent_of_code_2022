...
class CampCleanup
  ...
  def number_of_fully_overlapping_pairs
    elf_pairs.inject(0) do |sum, elf_pair|
      sum += 1 if fully_overlapping_pairs?(elf_pair)
      sum
    end
  end
...
private
  ...
  def fully_overlapping_pairs?(elf_pair)
    return true if first_elfs_section(elf_pair).subset?(second_elfs_section(elf_pair))
    return true if second_elfs_section(elf_pair).subset?(first_elfs_section(elf_pair))
...
class TestCampCleanup < Minitest::Test
  def test_fully_overlapping_pairs
    ...
    assert_equal 2, CampCleanup.new(elf_pairs).number_of_fully_overlapping_pairs
  end
end
