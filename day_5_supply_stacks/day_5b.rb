...
private
  ...
  def move_crates_based_on(sanitized_instruction)
    ...
    stacks[to_stack].unshift(crates_to_move).flatten!
  end
...
class TestSupplyStack < Minitest::Test
  def test_final_stack_order
    ...
    assert_equal "MCD", SupplyStack.new(stack_input: stack_input, instructions: instructions).crate_ids_of_top_crates_in_final_stack_order
  end
end
